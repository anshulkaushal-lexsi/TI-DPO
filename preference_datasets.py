...
def get_pku(split: str, silent: bool = False, cache_dir: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    ...
    """
    print(f'Loading PKU dataset ({split} split) from Huggingface...')
    dataset = datasets.load_dataset('PKU-Alignment/PKU-SafeRLHF', split=split, cache_dir=cache_dir)
    print('done')

    data = defaultdict(lambda: defaultdict(list))
    
    for row in tqdm.tqdm(dataset, desc='Processing PKU', disable=silent):
        safe_0 = row["is_response_0_safe"]
        safe_1 = row["is_response_1_safe"]

        if not safe_0 and not safe_1:
            continue

        if safe_0 and not safe_1:
            chosen_id = 0
        elif safe_1 and not safe_0:
            chosen_id = 1
        else:
            chosen_id = row["better_response_id"]
    ...
    def collate_fn(batch):
        ...
        for k in batch[0].keys():
            if k.endswith('_input_ids') or k.endswith('_attention_mask') or k.endswith('_labels'):
                ...
                # Check if to_pad is empty or contains empty tensors
                if len(to_pad) == 0:
                    raise ValueError(f"Empty to_pad list for key {k}")
                
                # Check for empty tensors
                for i, tensor in enumerate(to_pad):
                    if tensor.numel() == 0:
                        raise ValueError(f"Empty tensor at index {i} for key {k}")
                ...
                try:
                    ...
                    if len(batch) == batch_size:
                        try:
                            ...
                            # Check batch size
                            if collated_batch['chosen_input_ids'].shape[0] > 0:
                    ...
                    try:
                        ...
                        if len(batch) == batch_size:
                            try:
                                collated_batch = collate_fn(batch)
                                # Check batch size
                                if collated_batch['chosen_input_ids'].shape[0] > 0 and collated_batch['rejected_input_ids'].shape[0] > 0:
                                    yield collated_batch
                            except Exception as e:
                                if not silent:
                                    print(f"Skipping invalid batch: {e}")
